...
class BookShelvesRepository {
  async create(bookShelf) {
    ...
    try {
      const sql = `
                ...
                VALUES (?, ?, ?, ?)
            `;
      const [result] = await pool.execute(sql, [bookShelf.user_id, bookShelf.book_id, bookShelf.shelf_name, bookShelf.description]);
      return {
        id: result.insertId,
        ...bookShelf
      };
  ...
  async update(id, updatedData) {
    ...
    try {
      ...
      values.push(id);
      const [result] = await pool.execute(`UPDATE bookshelves SET ${fields} WHERE id = ?`, values);
